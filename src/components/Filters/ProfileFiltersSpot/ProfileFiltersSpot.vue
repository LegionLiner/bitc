    ...
    <div class="profile-filters" ref="filterProfileRef">
        <div class="profile-filters__btn" @click="toggleShow">
          <span v-if="selectedFilter"><span class="profile-title">{{ $t('userFilters.title') }}</span>{{ " " + selectedFilter }}</span>
            <span v-else>{{ $t('userFilters.title') + " " + $t('userFilters.nochoise')
                }}</span>
        ...
        <div class="modal" v-if="isShow">
            <div class="modal__title">
                ...
                <div class="modal__add-profile" @click="newUserFilterSpot">+</div>
            </div>
            <div class="modal__profiles">
                <ProfileFiltersSpotRow v-for="row of profileFilter" :row="row" :input="row?.input"
                    :active="row.filterName === selectedFilter" @closeMenu="hideShow" />
            </div>
        </div>
        <div class="icon-save">
            <SaveGreen v-if="selectedFilter && filterStore.editedFilters.has(selectedFilter)" class="cursor" @click="filterStore.saveUserFilterSpot(filterStore.selectedUserFilterSpot)"></SaveGreen>
        </div>
    </div>
</template>
...
import ProfileFiltersSpotRow from './ProfileFiltersSpotRow.vue';
