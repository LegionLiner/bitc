<template>
    <div class="profile-filter" ref="filterProfileRowRef" :class="{ 'profile-filter__active': active }" @click="editUserFilterSpot">
        <input type="text" v-if="editToggle" v-model="row.filterName" ref="myInput" @blur.stop="editProfileFilterSpot"
        ...
        <div class="profile-filter__btn">
            ...
            <BasketGray class="cursor" @click.stop="deleteUserFilterSpot" />
        </div>
    </div>
    <Teleport to="body">
...
function editUserFilterSpot() { 
    filterStore.editUserFilterSpot(props.row)
    emit('closeMenu')
}
function deleteUserFilterSpot() {
    showDeleteModal.value = true;
    filterStore.deleteUserFilter(props.row.filterName, "SPOT")
}
function deleteFilter() {
    filterStore.deleteUserFilter(props.row.filterName, "SPOT")
}
function editProfileFilterSpot() {
    editToggle.value = false;
    if (props.input) {
        userStore.addUserFilterSpot(props.row)
    }
    else userStore.editProfileFilterSpot(inputValue.value, props.row.filterName)
}
function saveFilter() {
    filterStore.saveUserFilterSpot(props.row);
}
onMounted(() => {
